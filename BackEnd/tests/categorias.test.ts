...
describe('Categorías — GET', () => {
  ...
  it('rechaza un id no numérico', async () => {
    const res = await request(app).get('/api/categorias/abc');
...
describe('Categorías — DELETE', () => {
  it('elimina una categoría existente', async () => {
    ...
    const res = await request(app).delete(`/api/categorias/${creada.id}`);
    expect(res.status).toBe(204);

    const despues = await request(app).get(`/api/categorias/${creada.id}`);
